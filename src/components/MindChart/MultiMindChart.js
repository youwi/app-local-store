...
const buildMultiEdge=(arr)=>{
  let out=[]
  if(arr==null || arr.length==0) return out
  for(let currdata of arr){
    if(currdata.constructor==Array){
      for(let cdata of currdata){
        if(cdata&&cdata.constructor==Array){
          Array.prototype.push.apply(out ,buildMultiEdge(cdata))
        }
        if(cdata&&cdata.itemMultiParentIds) {
          let ids = cdata.itemMultiParentIds.split(",")
          for (let targetid of ids) {
            out.push({source: cdata.id, target: targetid})
          }
          Array.prototype.push.apply(out ,buildMultiEdge(cdata.children))
        }
      }
    }else{
      if(currdata.itemMultiParentIds){
        let ids=currdata.itemMultiParentIds.split(",")
        for(let targetid of ids){
          out.push({source:currdata.id,target: targetid})
        }
      }
      Array.prototype.push.apply(out ,buildMultiEdge(currdata.children))
    }
...
const Chart = createG2(chart => {
  ...
  let multiEdge=buildMultiEdge(chart.get('data').arr )
  Array.prototype.push.apply(edges,multiEdge)
  ...
  //   item.y=item.y+0.01*i
  //   item.x=item.x+0.01*i
  // })
...
export default class MultiMindChart extends React.Component {
  state={
